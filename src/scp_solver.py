...
def main():
    ...
    NavigatioN = 600 #number of time steps in a single solve
    ...
    xFinal = np.array([x_extract,y,theta,0,0,0,0]) + 10 * np.hstack((direction,np.array([0,0,0]))) + 10 * np.hstack((orth_direction,np.array([0,0,0])))
    print("target: ",xFinal)
